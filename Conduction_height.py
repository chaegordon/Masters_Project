...
""" need to calculate Temp_max for each height probably"""


#this is the elastic value of the paper, the plastic value is encorporated later
Temp_max = 7300
t_c = 5*10**-6
...
def collision_time(height):
    t_c = (1+e)*2.87*((M/2)**2)/float(R*E_m**2*((2*g*height)**(0.5))**0.2)
    return t_c

def f(t,height):
    t_c = collision_time(height)
    flux_time = k*((Temp_max*(np.sin((t*10**-6)*np.pi/t_c))**(0.57)) - d_Theta(T))
    return flux_time


h = np.linspace(0.1,1.4,10)

soln = []
max_T =[]

for height in h:
        fluxy = np.where(f(T,height) < k*(588 - d_Theta(T)), f(T,height), k*(588 - d_Theta(T)))
...
n_arrhenius = np.exp(-506/np.array(max_T))/np.exp(-506/np.amax(max_T))


plt.figure()
plt.plot(h,n_arrhenius)
plt.title('Normalised Arrhenius Factor vs Drop Height')
plt.xlabel('Drop Height / metres')
plt.ylabel('exp(- 506K / T) / exp(- 506K / max T)')
plt.savefig('Pore_dissipated_Arrhenius_Burst_height.png')
